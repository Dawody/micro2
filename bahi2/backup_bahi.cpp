...
if(current[y1][x1]==1)return "error";
...
////////////////////
//ofstream myfile;
myfile.open("bahi2/previous.txt");
for(int i = 0; i < 8; i++)
for (int j = 0; j < 8; j++)
...
getdata();
ifstream myfile;
myfile.open("bahi2/read.txt");
string st;
myfile>>st;
myfile.close();
ofstream myfile2;
myfile2.open("bahi2/stockfish.txt");
myfile2<<movepos(st);
myfile2.close();
//filldata();
currenttoprevious();
